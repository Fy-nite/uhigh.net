    ...
    public class Compiler
    {
        ...
        public async Task<bool> SaveCSharpCode(string sourceFile, string outputFolder)
        {
            ...
            try
            {
                ...
                // not needed here, handled in ProjectManager
        ...
        // The following project system methods have been moved to ProjectManager:
        // - CreateProject
        // - ListProjectInfo
...
{string.Join("\n", deps)}
  ...
  <PropertyGroup>
    ...
    <Version>{uhighProject.Version}</Version>
        ...
        /// <summary>
        /// Compiles the μHigh source to the specified target language.
        /// </summary>
        /// <param name="source">The μHigh source code</param>
        /// <param name="targetLanguage">The target language (e.g., "csharp", "javascript")</param>
        /// <param name="diagnostics">Diagnostics reporter</param>
        /// <param name="rootNamespace">Optional root namespace</param>
        /// <param name="className">Optional class name</param>
        /// <returns>The generated code as a string</returns>
        public string CompileToTarget(string source, string targetLanguage, DiagnosticsReporter? diagnostics = null, string? rootNamespace = null, string? className = null)
        {
            diagnostics ??= new DiagnosticsReporter(_verboseMode);

            if (_verboseMode)
                diagnostics.ReportInfo($"Compiling to target: {targetLanguage}");

            var generator = CodeGeneratorRegistry.GetGenerator(targetLanguage);
            if (generator == null)
                throw new Exception($"No code generator found for target: {targetLanguage}");
            ...
            return generator.Generate(ast, diagnostics, rootNamespace, className);
        }
...
{string.Join("\n", deps)}
  ...
  <PropertyGroup>
    ...
    <Version>{uhighProject.Version}</Version>
        ...
        /// </summary>
        public void ListAvailableTargets()
        {
            Console.WriteLine("Available Code Generation Targets:");
            Console.WriteLine("==================================");
            
            foreach (var info in CodeGeneratorRegistry.GetGeneratorInfo())
            {
                Console.WriteLine($"\nTarget: {info.Name}");
                Console.WriteLine($"  Description: {info.Description}");
                Console.WriteLine($"  Version: {info.Version}");
                
                if (info.SupportedFeatures.Any())
                {
                    Console.WriteLine($"  Features: {string.Join(", ", info.SupportedFeatures)}");
                }
                
                if (info.RequiredDependencies.Any())
                {
                    Console.WriteLine($"  Dependencies: {string.Join(", ", info.RequiredDependencies)}");
                }
            }
        }

        /// <summary>
        /// Compiles the to target file using the specified source file
        /// </summary>
        /// <param name="sourceFile">The source file</param>
        /// <param name="targetLanguage">The target language</param>
        /// <param name="outputFile">The output file</param>
        ...
        public async Task<bool> CompileToTargetFile(string sourceFile, string targetLanguage, string? outputFile = null)
        {
            ...
            {
                var source = await File.ReadAllTextAsync(sourceFile);
                var generatedCode = CompileToTarget(source, targetLanguage, diagnostics);
                ...
                if (outputFile == null)
                {
                    ...
                    outputFile = baseName + (generator?.FileExtension ?? ".txt");
                }

                await File.WriteAllTextAsync(outputFile, generatedCode);
                Console.WriteLine($"Generated {targetLanguage} code: {outputFile}");
            ...
            {
                diagnostics.ReportFatal($"Failed to compile to {targetLanguage}: {ex.Message}", exception: ex);
                diagnostics.PrintSummary();
                return false;
            }
        }
